(function () {
  ...
  function shouldIncludeNewsForUser(row, user) {
  ...
  async function showNewsSince(previousLoginTs, user) {
    ...
    const { data, error } = await supabaseClient
      .from("gdb_log")
      .select("timestamp, user_id, username, action, item_type, item_name, details")
    ...
    const filteredRows = (data || []).filter((row) => shouldIncludeNewsForUser(row, user));
    renderNewsItems(filteredRows);
    openNewsModal();
  }
